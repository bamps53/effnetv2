...
DOWNLOAD_DIR = 'https://github.com/bamps53/effnetv2/releases/download/beta/'
MODEL_NAMES = [
...
class MBConvBlock(tf.keras.layers.Layer):
    ...
    def _build(self):
        ...
        if self._has_se:
            ...
            self._se = SE(self._mconfig, num_reduced_filters,
                          filters, name='se')
        else:
            self._se = None
    ...
    def call(self, inputs, training, survival_prob=None):
        ...
        if self._mconfig.conv_dropout and self._block_args.expand_ratio > 1:
            x = tf.keras.layers.Dropout(
                self._mconfig.conv_dropout)(x, training)
...
class Head(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training):
        """Call the layer."""
        outputs = self._act(self._norm(
            self._conv_head(inputs), training=training))
...
class EffNetV2Model(tf.keras.Model):
    ...
    def _build(self):
        ...
        # Stem part.
        self._stem = Stem(
            self._mconfig, self._mconfig.blocks_args[0].input_filters)
        ...
        for block_args in self._mconfig.blocks_args:
            ...
            input_filters = round_filters(
                block_args.input_filters, self._mconfig)
            output_filters = round_filters(
                block_args.output_filters, self._mconfig)
